...
def gradient_text(text, font, color_start, color_end):
    """Return RGBA image of text with horizontal gradient."""
    tmp = Image.new("RGBA", (1,1)); td = ImageDraw.Draw(tmp)
    total_w = int(td.textlength(text, font=font))
    h = int(font.size * 1.5)
    out = Image.new("RGBA", (total_w + 4, h), (0,0,0,0))
    od = ImageDraw.Draw(out)
    cx = 0
    for ch in text:
        cw = int(td.textlength(ch, font=font))
        t = cx / total_w if total_w else 0
        ...
        od.text((cx, 0), ch, fill=(r,g,b,255), font=font)
        cx += cw
    return out
...
def make_og(meta, fv_path, fm_path, out):
    ...
    fb=ImageFont.truetype(fm_path,15*SCALE); fmt=ImageFont.truetype(fm_path,14*SCALE)
    ft=ImageFont.truetype(fv_path,64*SCALE); fs=ImageFont.truetype(fm_path,21*SCALE)
    fbr=ImageFont.truetype(fv_path,22*SCALE); fu=ImageFont.truetype(fm_path,13*SCALE)
    fver=ImageFont.truetype(fm_path,14*SCALE)
    px=cx1+50*SCALE; bx,by=px,cy1+60*SCALE
    ...
    y=cy1+135*SCALE
    for line in textwrap.wrap(meta["title"],width=26)[:2]:
        d.text((px,y),line,fill=WHITE,font=ft); y+=82*SCALE
    if meta["accent"]:
        gtxt = gradient_text(meta["accent"],ft,(255,255,255),(249,115,22))
        img.paste(gtxt,(px,y),gtxt); y+=82*SCALE
    y+=12*SCALE
    for line in textwrap.wrap(meta["subtitle"],width=60)[:2]:
        d.text((px,y),line,fill=GRAY,font=fs); y+=34*SCALE
    div_y=cy2-70*SCALE
